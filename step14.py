...
if "Chat between a customer and a salon owner" not in schema:
    schema = schema.rstrip("\n") + "\n" + new_sql
    with open(schema_path, "w") as f:
        f.write(schema)
    changes.append("✅ schema.sql updated with conversations + messages tables")
else:
    changes.append("⏭️  schema.sql already updated — skipped")

conversations_path = "backend/routes/conversations.js"
conversations_content = '''const express = require("express");
...
if not os.path.exists(conversations_path):
    with open(conversations_path, "w") as f:
        f.write(conversations_content)
    changes.append("✅ Created backend/routes/conversations.js")
else:
    changes.append("⏭️  conversations.js already exists — skipped")

server_path = "backend/server.js"
with open(server_path, "r") as f:
    server = f.read()

old_require = 'const userRoutes = require("./routes/users");'
new_require = 'const userRoutes = require("./routes/users");\nconst conversationRoutes = require("./routes/conversations");'
old_use = 'app.use("/users", userRoutes);'
new_use = 'app.use("/users", userRoutes);\napp.use("/conversations", conversationRoutes);'

if "conversationRoutes" not in server:
    assert server.count(old_require) == 1
    assert server.count(old_use) == 1
    server = server.replace(old_require, new_require)
    server = server.replace(old_use, new_use)
    with open(server_path, "w") as f:
        f.write(server)
    changes.append("✅ server.js — registered /conversations routes")
else:
    changes.append("⏭️  server.js already registered — skipped")
