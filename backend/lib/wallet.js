...
async function creditWallet(userId, amount, { type = "refund", bookingId = null, orderId = null } = {}) {
  await db.query("UPDATE users SET wallet_balance = wallet_balance + $1 WHERE id = $2", [amount, userId]);
  await db.query(
    `INSERT INTO wallet_transactions (user_id, type, amount, booking_id, order_id, status) VALUES ($1, $2, $3, $4, $5, 'success')`,
    [userId, type, amount, bookingId, orderId]
  );
}
