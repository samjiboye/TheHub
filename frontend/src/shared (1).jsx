...
function SalonCard({ salon, onClick }) {
  ...
  return (
    <button
      onClick={onClick}
      className="text-left rounded-2xl overflow-hidden w-full flex items-center gap-3 px-3 py-2.5 transition-transform active:scale-[0.98]"
    ...
    >
      ...
      >
        {salon.profile_image_url ? (
          ...
          <Icon size={26} strokeWidth={1.6} color={colors.hairline} />
        )}
      </div>
      ...
      <span
        className="shrink-0 w-8 h-8 rounded-full flex items-center justify-center tap-glass"
